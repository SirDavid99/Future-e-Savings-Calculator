        ...
        monthlySavingInput.addEventListener('input', function() {
            if (!document.getElementById('groupSaving').checked) {
                ...
                if (monthlySaving > 0) {
                    ...
                    document.getElementById('monthlySavingDisplay').textContent = formatCurrency(0);
                    document.getElementById('monthsDisplay').textContent = `${months} mesi`;
                    document.getElementById('yearsDisplay').textContent = `${years} anni`;
                }
            }
...
function updateResultsInRealTime(monthlyTotal, peopleCount, perPersonAmount) {
    ...
    const perPersonTotal = calculateTotalSaving(perPersonAmount);
...
function getYearsCount() {
    const yearsInput = document.getElementById('yearsCount');
    return yearsInput ? parseInt(yearsInput.value) || 5 : 5;
}
...
function reorderNames(names) {
    ...
    // Se tutti e quattro i nomi sono presenti, riordina: L, X, Y, D
    if (dIndex !== -1 && lIndex !== -1 && xIndex !== -1 && yIndex !== -1) {
        return ['L', 'X', 'Y', 'D'];
    }
    
    // Se alcuni nomi sono presenti, costruisci l'array nell'ordine corretto
    const reordered = [];
    if (lIndex !== -1) reordered.push('L');
    if (xIndex !== -1) reordered.push('X');
    if (yIndex !== -1) reordered.push('Y');
    if (dIndex !== -1) reordered.push('D');
    
    // Aggiungi eventuali altri nomi non standard
    names.forEach(name => {
    ...
    // Se abbiamo meno di 4 nomi, aggiungi i default mancanti nell'ordine corretto
    if (reordered.length < 4) {
        if (!reordered.includes('L')) reordered.unshift('L');
        if (!reordered.includes('X')) {
            const lPos = reordered.indexOf('L');
            reordered.splice(lPos + 1, 0, 'X');
        }
        if (!reordered.includes('Y')) {
            const xPos = reordered.indexOf('X');
            reordered.splice(xPos + 1, 0, 'Y');
        }
        if (!reordered.includes('D')) reordered.push('D');
    }
...
function updateIndividualSavers(perPersonAmount, investorNames) {
    ...
    // Estrai i nomi e riordina per mettere X al centro, Y a destra di X, e L a destra di Y
    ...
    for (let i = 0; i < 4; i++) {
        const name = names[i] || `Persona ${i + 1}`;
...
function updateResult() {
    ...
    // Se non c'è risparmio mensile, resetta tutto
...
function updateGroupResult(data) {
    ...
    const membersList = document.getElementById('groupMembersList');
    let names = ['D', 'L', 'X'];
    if (membersList && data.investorNames) {
...
document.getElementById('savingForm').addEventListener('submit', function(e) {
    ...
    let perPersonAmount = null;
    let investorNames = null;
    
    if (isGroup) {
        peopleCount = parseInt(document.getElementById('peopleCount').value);
        perPersonAmount = parseFloat(document.getElementById('perPersonAmount').value);
        ...
        if (!perPersonAmount || perPersonAmount <= 0) {
            alert('Inserisci un contributo per persona valido');
            return;
        }
        
        monthlySaving = peopleCount * perPersonAmount;
        investorNames = document.getElementById('investorNames').value || 'L, X, Y, D';
    } else {
        monthlySaving = parseFloat(document.getElementById('monthlySaving').value);
        
        if (!monthlySaving || monthlySaving <= 0) {
            alert('Inserisci un risparmio mensile valido');
            return;
        }
    }
    ...
    const totalSaving = calculateTotalSaving(monthlySaving);
    const groupText = isGroup ? ` (${peopleCount} persone × ${formatCurrency(perPersonAmount)})` : '';
    alert(`✅ Risparmio calcolato!\n\nRisparmio mensile: ${formatCurrency(monthlySaving)}${groupText}\n\nTra ${yearsCount} anni (${months} mesi) risparmierai: ${formatCurrency(totalSaving)}`);
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Rimuovi valori vecchi dal localStorage e aggiorna
    if (savedData) {
        let updated = false;
        
        // Aggiorna perPersonAmount da 150 a 200
        if (savedData.perPersonAmount === 150) {
            savedData.perPersonAmount = 200;
            updated = true;
        }
        
        // Aggiorna peopleCount da 3 a 4
        if (savedData.peopleCount === 3) {
            savedData.peopleCount = 4;
            updated = true;
        }
        
        // Salva i dati aggiornati se ci sono state modifiche
        if (updated) {
            saveMonthlySaving(savedData);
        }
    }
    
    // Aggiorna i nomi se necessario (sostituisce Simone/Michela con Leo e Pietro con X)
    if (savedData) {
        savedData = updateSavedNames(savedData);
        ...
        if (savedData.isGroup) {
            ...
            document.getElementById('peopleCount').value = savedData.peopleCount || 4;
            document.getElementById('perPersonAmount').value = savedData.perPersonAmount || 200;
            
            // Assicurati che i nomi siano aggiornati e riordinati
            let investorNames = savedData.investorNames || 'L, X, Y, D';
            if (typeof investorNames === 'string') {
                investorNames = investorNames
                    .replace(/Simone/gi, 'L')
                    .replace(/Michela/gi, 'L')
                    .replace(/Pietro/gi, 'X')
            ...
            // Riordina i nomi per assicurarsi che L sia a sinistra, X al centro, Y a destra di X, e D a destra di Y
            const reorderedNames = reorderNames(investorNames);
            document.getElementById('investorNames').value = reorderedNames.join(', ');
            updateCalculatedTotal();
        } else {
            document.getElementById('monthlySaving').value = savedData.monthlySaving || '';
        }
    } else {
        // Se non ci sono dati salvati, inizializza con i valori di default
        const peopleCountInput = document.getElementById('peopleCount');
        const perPersonAmountInput = document.getElementById('perPersonAmount');
        if (peopleCountInput && perPersonAmountInput) {
            // Assicurati che i valori di default siano corretti
            if (!peopleCountInput.value || parseInt(peopleCountInput.value) === 0) {
                peopleCountInput.value = 4;
            }
            if (!perPersonAmountInput.value || parseFloat(perPersonAmountInput.value) === 0) {
                perPersonAmountInput.value = 200;
            }
            // Calcola il totale con i valori di default
            updateCalculatedTotal();
        }
    }
    
    // Assicurati che il campo risparmiatori abbia sempre l'ordine corretto: L, X, Y, D
    const investorNamesInput = document.getElementById('investorNames');
    if (investorNamesInput) {
        // Sostituisci sempre Pietro con X, Leo con L, Davide con D se presenti
        if (investorNamesInput.value) {
        ...
        // Se il campo è vuoto o contiene nomi vecchi, imposta il default
        if (!investorNamesInput.value || 
            investorNamesInput.value.includes('Simone') || 
            investorNamesInput.value.includes('Michela') ||
            investorNamesInput.value.includes('Pietro') ||
            investorNamesInput.value.includes('Leo') ||
            investorNamesInput.value.includes('Davide')) {
            investorNamesInput.value = investorNamesInput.value || 'L, X, Y, D';
            investorNamesInput.value = investorNamesInput.value
                .replace(/Simone/gi, 'L')
                .replace(/Michela/gi, 'L')
                .replace(/Pietro/gi, 'X')
        ...
        // Riordina sempre i nomi per assicurarsi che L sia a sinistra, X al centro, Y a destra di X, e D a destra di Y
